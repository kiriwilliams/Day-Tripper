$(document).ready(function () {
    ...
    $("#updateZipcode").on("click", function (event) {
        ...
        checkZip($("#zipcode").val().trim());
    });


    function checkZip(value) {
        const zipFormat = (/(^\d{5}$)|(^\d{5}-\d{4}$)/).test(value);
        if (zipFormat) {
    ...
    async function yelpHandler() {
        ...
        Promise.all(categoryCalls).then(function (categoryArray) {
            ...
            yelpResultsValues = Object.values(yelpResults);

            //if there are results
            if (yelpResultsValues.length > 0) {
                ...
                $("#yelpFeedback").text("No results found in the yelp API.")
            }
    ...
    $("#submit").on("click", function (e) {
        e.preventDefault();
        //empty out yelp feedback, previous results
        $("#yelpFeedback").empty();
        $("#results").empty();
        yelpResultsValues = [];
